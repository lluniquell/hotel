...
@Service
@Qualifier("tps")
public class TeamproServiceImpl_ad implements TeamproService_ad {
	...
	public String qna_list(HttpSession session, Model model, HttpServletRequest request)
	{
		...
		ArrayList<QnaVo> list=tMapper.qna_list_ad(index);
		model.addAttribute("list", list);			
		
		return "/ad_qna/qna_list";
	}
	...
	public String qna_delete(HttpServletRequest request)
	{
		...
		return "redirect:/ad_qna/qna_list";
	}
	...
	public String qna_answer_ok(AnswerVo avo, HttpSession session, Model model)
	{
		...
		avo.setUserid("java000"); //임시 삭제하고 위에 살리기
		
		// 문의 답변 작성 (저장)
		tMapper.qna_answer_ok(avo);
				
		// 문의 상태 0 => 1
		tMapper.change_state(avo.getQid());				
		
		return "redirect:/ad_qna/qna_content?id="+avo.getQid();
	}	
	
	@Override /*1:1문의 답변 수정*/
	public String answer_update_ok(AnswerVo avo)
	{
		tMapper.answer_update(avo);		
...
// faq 자주묻는 질문
	...
	public String faq_write_ok(HttpSession session,FaqVo fvo)
	{
	   ...
	   tMapper.faq_write_ok(fvo);

	   return "redirect:/ad_faq/faq_list";
    ...
    public String faq_update(HttpServletRequest request, Model model)
    {
		...
		FaqVo fvo=tMapper.faq_update(id);
		model.addAttribute("fvo", fvo);
		return "/ad_faq/faq_update";		
    }
	
	@Override // faq 수정 완료
	public String faq_update_ok(FaqVo fvo)
	{
		tMapper.faq_update_ok(fvo);
...
// 이용 후기
	...
	public String hugi_readnum(HttpServletRequest request)
	{
		...
		return "redirect:/ad_hugi/hugi_content?id="+id;
	}
	...
	public String hugi_delete(HttpServletRequest request)
	{
		...
		return "redirect:/ad_hugi/hugi_list";
	}
...
/*상품 리뷰*/
	...
	{		
		...
		String userid="userid"; /*String userid=session.getAttribute("userid").toString();*/
