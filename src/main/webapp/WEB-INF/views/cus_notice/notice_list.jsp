  ...
  <h1 class="mt-4" style="text-align:center">공지사항</h1> 
    <table class="table table-hover"> 
     ...
     <tbody>
       <c:forEach items="${notice_list}" var="nvo">
         <tr>
           <td><a href="../cus_notice/notice_readnum?id=${nvo.id}&page=${page}&sel=${sel}&sword=${sword}" style="color:#000;">${nvo.title}</a></td>
           <td>${nvo.writeday}</td>
           <td>${nvo.readnum}</td>
         </tr>
        </c:forEach>
   ...
   <table style="margin-left:auto;margin-right:auto;">
       <tr class="link">
         ...
         <!-- 10페이지 이전으로 이동 -->
         <c:if test="${pstart != 1}">
           <a href="../cus_notice/notice_list?page=${pstart-1}&sel=${sel}&sword=${sword}" style="color:#000;"> << </a>
         </c:if>
         <c:if test="${pstart == 1}"> << </c:if>
            
         <!-- 1페이지 이전으로 이동 -->
         <c:if test="${page != 1}">
          <a href="../cus_notice/notice_list?page=${page-1}&sel=${sel}&sword=${sword}" style="color:#000;"> < </a>
         </c:if>
         <c:if test="${page == 1}"> < </c:if>
 
         <c:forEach begin="${pstart}" end="${pend}" var="i">
         <c:if test="${page == i}">
          <a href="../cus_notice/notice_list?page=${i}&sel=${sel}&sword=${sword}" style="background:#a17e48e0; margin-bottom:20px; height:25px;">${i}</a>
         </c:if>
         <c:if test="${page != i}">
          <a href="../cus_notice/notice_list?page=${i}&sel=${sel}&sword=${sword}">${i}</a>
         </c:if>
         </c:forEach>
         <!-- 1페이지  이동 시작-->
